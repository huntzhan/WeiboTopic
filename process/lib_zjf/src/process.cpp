...
void Process::goodWordsinPieceArticle(std::list<std::string> &goodword){
  ...
  boost::split(goodWordstemp,wrtesult ,boost::is_any_of("|"));




  for(std::list<std::wstring>::iterator it=goodWordstemp.begin();it!=goodWordstemp.end();it++){



   // std::string temp=WidetoString(*it);
...
std::wstring Process::StringToWide(std::string &sToMatch){
	...
	wchar_t *lpwsz= new wchar_t[iWLen+100];
...
std::string Process::Regex_Replace(std::string input){
	...
	input+="  "; //这里是处理空字符 就是只有@ 吧在stopword去掉
