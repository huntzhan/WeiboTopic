		...
		for(;it_subTopic!=end_subTopic;it_subTopic++){
			...
			for(unsigned int i:hashVector){
				if(TagHash.count(i)){
					 ...
					 printf("微博热门话题: %12s   OneDayTopic  %30s     the id is %d \n",it->second.c_str(),it_subTopic->c_str(),it_topic->id);
...
void CCoverage::GetSearchCount(void){
...
void CCoverage::SearchCountSort(){
	...
	for(;it_searchcount!=end_count;it_searchcount++){
		cout<<it_searchcount->hottopic<<"-------"<<it_searchcount->count<<endl;
	}
}
/*
 * 输入两个短文本进行
		...
		for(;it_subTopic!=end_subTopic;it_subTopic++){
			...
			for(;it_tag!=end_tag;it_tag++){
				...
				if(ja>=JaccardThrethod){
					 cout<<ja<<endl;
					 printf("微博热门话题: %-50s   OneDayTopic  %-50s     the id is %d \n",it_tag->second.c_str(),it_subTopic->c_str(),it_topic->id);
...
void CCoverage::MatchTopicByJaccard(const std::string hotTopic){
//	 ///提取包含tag的文本
//	 GetTag();
//	 ///对tag进行排序
//	 SortTag();
	 ...
	 DisplayMatch();
	 cout<<"准确率 ： "<<double(matchTopic.size())/topicMainIdea.size()<<"   覆盖率 :"<<double(matchTopic.size())/TagHash.size()<<endl;
}
...
void CCoverage::DisplayMatch(){
    ...
    for(;it_match!=end_match;it_match++){
    	cout<<"topic ID "<<it_match->second.id<<"-----------------匹配对应的话题  ：          "<<it_match->second.matchstring<<endl;
    }
}
